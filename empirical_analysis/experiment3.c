 ...
 * iteration budget?
 *
 *     Fixes k = 100 and epsilon = 0.5. Varies polynomial degree from
 *     2 to 10 (n_params = 3 to 11). True params are [1, 1, ..., 1]
 *     for all degrees. N_TOTAL = 1000 is fixed throughout. x is scaled
 *     to [0, 1] to avoid Vandermonde ill-conditioning with float.
 *     Repeats N_REPEATS times per degree.
 *
...
int main(void) {
    ...
    for (int deg = DEGREE_MIN; deg <= DEGREE_MAX; deg++) {
        ...
        for (int r = 0; r < N_REPEATS; r++) {
            ...
            /* write one CSV row */
            fprintf(fp, "%d,%d,%.1f,%d,%d,%d,%d,%.4f,%d,%.2f,%.6f\n",
    ...
    fclose(fp);
    printf("Experiment 3 done. Results written to %s\n", EXP3_CSV);
    return 0;
}