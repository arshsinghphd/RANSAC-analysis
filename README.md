...
2. If $len(S_1) * \ge expected\_inliers$, where $d$ is a threshold derived from the estimated outlier fraction $\varepsilon$, use $$S_1^*$$ to compute a refined model $$M_1^*$$ using least squares over all consensus points. Return $$M_1^*$$ as the best model.
